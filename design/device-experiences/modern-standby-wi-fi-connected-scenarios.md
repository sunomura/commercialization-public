...
You can use the test Microsoft account to configure the system with a Lync and Skype account. To test the instant messaging scenario, make sure the system is put in modern standby. After system is in modern standby, send an instant messages to the test account. You should observe that system powers on the display when instant message is received and shows an instant message toast notification on the lock screen.
